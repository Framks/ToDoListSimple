...
public class GlobalExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    protected ResponseEntity<Object> handleMethodArgumentNotValid(MethodArgumentNotValidException method,
                                                                  HttpHeaders headers,
        ...
        for (FieldError fieldError: method.getBindingResult().getFieldErrors())
            errorResponse.addValitadionError(fieldError.getField(),fieldError.getDefaultMessage());
        return ResponseEntity.unprocessableEntity().body(errorResponse);
    }

    @ExceptionHandler(Exception.class)
    @ResponseStatus(HttpStatus.INTERNAL_SERVER_ERROR)
    public ResponseEntity<Object> handleAllUncaughtException1(Exception exception,
                                                              WebRequest request) {
        final String errorMessage = "unkown error eccurred";
    ...
    public ResponseEntity<Object> handleDataIntegrityViolationException(DataIntegrityViolationException dataIntegrityViolationException,
        ...
        log.error("Failed to sabe entity with integrity problem: "+errorMessage, dataIntegrityViolationException);
    ...
    public ResponseEntity<Object> handleObjectNotFoundException(ObjectNotFoundException objectNotFoundException,
        ...
        return  buildErrorResponse(objectNotFoundException,
    ...
    @ResponseStatus(HttpStatus.CONFLICT)
    public ResponseEntity<Object> handleDataBindinViolarionException(DataBindingViolationException dataBindingViolationException,
                                                                     WebRequest request){
        log.error("Failed to save entity with associated data", dataBindingViolationException);
        return buildErrorResponse(dataBindingViolationException,
                                  HttpStatus.CONFLICT,
                                  request);
    }
